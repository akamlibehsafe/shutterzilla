...
This document outlines suggested data models for the ShutterZilla platform. These are recommendations and can be adapted during implementation.
...
| `condition` | `enum` | The condition of the camera (e.g., `mint`, `excellent`, `good`). |
| `is_sold` | `boolean` | Whether the camera has been marked as sold. |
| `posted_at` | `timestamp` | The timestamp when the listing was posted. |
| `scraped_at` | `timestamp` | The timestamp when the listing was scraped. |
...
| `purchase_price` | `decimal` | The price the user paid for the camera. |
| `current_value` | `decimal` | The estimated current value of the camera. |
| `notes` | `text` | User's personal notes about the camera. |
| `image_url` | `string` | URL of an image of the camera. |
